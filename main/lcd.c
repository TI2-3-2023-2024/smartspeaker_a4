...
//    hd44780_upload_character(&lcd, 2, tuner);
    ...
    while (1)
    {
        ...
        write_string_on_pos(&lcd, 6, 1, strftime_buf);

        vTaskDelay(pdMS_TO_TICKS(500));
    }
}

void connection_status(void *pvParameters) {
    // LCD properties
    hd44780_t lcd = {
            .write_cb = write_lcd_data, // use callback to send data to LCD by I2C GPIO expander
            .font = HD44780_FONT_5X8,
    ...
    for(;;) {
        hd44780_clear(&lcd);
        if(pvParameters == 0) {
            write_string_on_pos(&lcd, 0, 0, "Connecting to WiFi..");
            vTaskDelay( 1500 / portTICK_PERIOD_MS);
            break;
        }
        if(pvParameters == 1) {
            write_string_on_pos(&lcd, 0, 0, "Connection Failed...");
            write_string_on_pos(&lcd, 0, 1, "RETRYING");
            vTaskDelay( 1500 / portTICK_PERIOD_MS);
            break;
        }
        if(pvParameters == 3) {
            write_string_on_pos(&lcd, 0, 0, "Time sync successful");
            vTaskDelay( 1500 / portTICK_PERIOD_MS);
            break;
        }
