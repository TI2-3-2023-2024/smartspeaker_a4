...
void RequestWeather(char* location, char* status, float* temp, char* output_buffer){
    ...
    ParseJSON(location, status, temp, output_buffer);
}
...
void ParseJSON(char* location, char* status, float* temp, char* output_buffer){
    ...
    if (root_value == NULL) {
        ...
        ParseJSON(location, status, temp, json_test);
    }else{        
        ...
        strcpy(status, json_object_get_string(weather_array_object, "samenv"));
        *temp = (float)atof(json_object_get_string(weather_array_object, "temp"));
